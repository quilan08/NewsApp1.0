...
import com.loc.newsapp.presentation.details.Screens.DetailsScreens
...
fun NewsNavigator() {
    ...
    val isBottonBarVisible = remember (key1 = backStackState){
        backStackState?.destination?.route ==Route.HomeScreen.route ||
                backStackState?.destination?.route ==Route.SearchScreen.route ||
                backStackState?.destination?.route ==Route.BookmarkScreen.route
    }
    Scaffold(modifier = Modifier.fillMaxSize(), bottomBar = {
        if (isBottonBarVisible){
        ...
        NavHost(
            ...
            composable(route = Route.HomeScreen.route) {
                ...
                HomeScreen(
                    articles = articles,
                    navigateToSearchScreen = { navigateToTab(navController = navController, route = it) },
                    navigateToDetailScreen = { article -> navigateToDetails(navController = navController,article= article) }
                    )
            }
            ...
            composable(route = Route.DetailsScreen.route) {
                ...
                if(viewModel.sideEffects != null){
                    Toast.makeText(LocalContext.current, viewModel.sideEffects, Toast.LENGTH_SHORT).show()
                    viewModel.onEvent(DetailsEvents.RemoveSideEffects)
                }
                navController.previousBackStackEntry?.savedStateHandle?.get<Article?>("article").let {
                    article ->
                    if (article != null) {
                        DetailsScreens(article = article, event = viewModel::onEvent, navigateUp ={
